...
import { EMBED_PROVIDER_TYPES } from '../../types/embed'
...
interface Props {
  ...
  onEdit: (p: EmbedProvider) => void
  onDelete: (p: EmbedProvider) => void
  onTest: (p: EmbedProvider) => void
  onSetDefault: (p: EmbedProvider) => void
  testing?: boolean
}

export default function EmbedProviderCard({ provider, onEdit, onDelete, onTest, onSetDefault, testing }: Props) {
  const typeLabel = EMBED_PROVIDER_TYPES.find((t) => t.value === provider.provider_type)?.label || provider.provider_type
  ...
  return (
    <Card
      ...
      style={{
        marginBottom: 12,
        borderRadius: 10,
        border: provider.is_default ? '2px solid #0891b2' : '1px solid #e2e8f0',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            ...
            {provider.is_default && (
              <Tag color="cyan" style={{ fontSize: 11 }}>默认</Tag>
            )}
            {!provider.is_active && (
        ...
        <Space>
          <Tooltip title={provider.is_default ? '当前默认' : '设为默认'}>
            <Button
              type="text"
              size="small"
              icon={provider.is_default ? <StarFilled style={{ color: '#0891b2' }} /> : <StarOutlined />}
