import { Card, Tag, Button, Popconfirm, Typography, Space, Tooltip, Badge } from 'antd'
import {
  DeleteOutlined,
  EditOutlined,
  CheckCircleOutlined,
...
export default function ProviderCard({ provider, onEdit, onDelete, onTest, onSetDefault, testing }: Props) {
  ...
  return (
    <Card
      ...
      style={{
        marginBottom: 12,
        borderRadius: 10,
        border: provider.is_default ? '2px solid #4f46e5' : '1px solid #e2e8f0',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            ...
            {provider.is_default && (
              <Tag color="purple" style={{ fontSize: 11 }}>默认</Tag>
            )}
            {!provider.is_active && (
        ...
        <Space>
          <Tooltip title={provider.is_default ? '当前默认' : '设为默认'}>
            <Button
              type="text"
              size="small"
              icon={provider.is_default ? <StarFilled style={{ color: '#4f46e5' }} /> : <StarOutlined />}
