...
import { OCR_PROVIDER_TYPES, OCR_BASE_URL_MAP } from '../../types/ocr'
import { ocrProvidersApi } from '../../api/ocrProviders'

interface Props {
  open: boolean
  onClose: () => void
  onSubmit: (data: OCRProviderCreate) => Promise<void>
  initialValues?: OCRProvider | null
  loading?: boolean
}

export default function OCRProviderForm({ open, onClose, onSubmit, initialValues, loading }: Props) {
  ...
  const handleProviderTypeChange = (value: string) => {
    const baseUrl = OCR_BASE_URL_MAP[value]
  ...
  const handleFetchModels = async () => {
    ...
    try {
      const models = await ocrProvidersApi.fetchModels({
  ...
  return (
    <Modal
      title={isEdit ? '编辑 OCR 视觉提供商' : '添加 OCR 视觉提供商'}
      open={open}
      onOk={handleOk}
      onCancel={onClose}
      ...
      <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
        ...
        >
          <Select
            showSearch
            allowClear
            placeholder="选择或输入，如 qwen-vl-max、gpt-4o"
